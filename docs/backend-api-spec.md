...
```json
{
  ...
  "message": "请求参数不合法",
  "errors": [
    { "field": "username", "message": "用户名不能超过 15 个 UTF-8 字节" }
  ],
  "requestId": "req_01K..."
}
```
...
- 用户名只允许英文字母和 `U+3400-U+9FFF` 范围内的中文字符。
- 用户名 UTF-8 编码长度为 1–15 字节。当前前端通过 `encodeURIComponent` 计算，中文通常占 3 字节。
...
- [ ] 中文用户名按 UTF-8 字节计算，5 个常用汉字通常正好 15 字节。
- [ ] 密码规则由后端再次校验，不能只依赖前端。
