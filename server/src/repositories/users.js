...
async function create(user) {
  await pool.execute(
    `INSERT INTO users
      (id, display_name, account_code, username, username_normalized, password_hash, timezone, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [user.id, user.displayName, user.accountCode, user.username, user.usernameNormalized, user.passwordHash,
      user.timezone, user.status]
  )
}

async function findForLogin(accountCode) {
  const [rows] = await pool.execute(
    `SELECT id, display_name, account_code, password_hash, avatar_url, timezone, status, deleted_at
     FROM users WHERE account_code = ? LIMIT 1`,
    [accountCode]
  )
  return rows[0] || null
}
...
async function findActiveById(id) {
  const [rows] = await pool.execute(
    `SELECT id, display_name, account_code, password_hash, avatar_url, timezone
